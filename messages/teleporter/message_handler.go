...
type factory struct {
	...
	deciderClient   pbDecider.DeciderClient
}
...
func NewMessageHandlerFactory(
	...
	var deciderClient pbDecider.DeciderClient
	if grpcClient == nil {
		deciderClient = nil
	} else {
		deciderClient = pbDecider.NewDeciderClient(grpcClient)
	}
...
func (m *messageHandler) ShouldSendMessage(destinationClient vms.DestinationClient) (bool, error) {
	...
	if deciderClientValue.IsValid() && !deciderClientValue.IsNil() {
		id, err := hex.DecodeString(m.unsignedMessage.ID().Hex())
		if err != nil {
			m.logger.Error(
